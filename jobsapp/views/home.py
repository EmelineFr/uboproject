...
class ApplyJobView(CreateView):
    ...
    def post(self, request, *args, **kwargs):
        form = self.get_form()
        if form.is_valid():
            messages.info(self.request, 'Vous avez bien postulé à l offre')
    ...
    def form_valid(self, form):
        # regarder si le postulant à déjà postulé
        applicant = Applicant.objects.filter(user_id=self.request.user.id, job_id=self.kwargs['job_id'])
        if applicant:
            messages.info(self.request, 'Vous avez déjà postulé pour cette offre')
